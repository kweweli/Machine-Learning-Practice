...
def normalize(X):
    ...
    """
    mu = np.mean(X, axis= 0 ,dtype=np.float64) # <-- EDIT THIS
    std = np.std(X, axis=0)
    std_filled = std.copy()
    std_filled[std==0] = 1.
    Xbar =  preprocessing.scale(X)                # <-- EDIT THIS
    return Xbar, mu, std
...
def PCA(X, num_components):
    ...
    U = X.T @ eigvecs
    P = projection_matrix(U[:, 0:num_components])
    pca = P @ X.T
    return pca.T
...
def PCA_high_dim(X, n_components):
    ...
    eigvals, eigvecs = eig(M)
    U = X.T @ eigvecs
    P = projection_matrix(U[:, 0:n_components])
    pca = P @ X.T
    return pca.T
