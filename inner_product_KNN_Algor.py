...
def distance(x0, x1):
    ...
    dx       = x0 - x1
    distance = np.sqrt(dx.T @ dx) # <-- EDIT THIS
    return distance

def angle(x0, x1):
    """Compute the angle between two vectors x0, x1 using the dot product"""
    angle = (x0.T @ x1)/np.sqrt((x0.T @ x0)*(x1.T @ x1)) # <-- EDIT THIS
...
for i in range(len(images[:500])):
    for j in range(len(images[:500])):
        distKwe.append(distance(images[i], images[j]))
farthestNeigh = 80000000000 
counter = 0
for i in range(1,500):
    if distKwe[i] < farthestNeigh:
        farthestNeigh = distKwe[i]
        counter += 1
...
def most_similar_image():
    ...
    """
    index = counter #<-- Change the -1 to the index of the most similar image.
...
def pairwise_distance_matrix(X, Y):
    ...
    M, D = X.shape
    x = X
    y = np.repeat(Y, D).reshape(-1, D)
    difxy = x - y
    distance_matrix = np.sqrt(difxy.T @ difxy)# <-- EDIT THIS
...
def KNN(k, X, y, x):
    ...
    dist = pairwise_distance_matrix(X, y) #.flatten()# <-- EDIT THIS
    ...
    for c in np.unique(classes):
        ypred[c] = len(classes[classes ==c])/k  # <-- EDIT THIS
